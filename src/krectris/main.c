...
static void detirmine_ghost_position() {
	...
	for (int i = 0; i <= 20; i++) {
		if (verify_piece_placement(ghost_piece_x, ghost_piece_y + 1, ghost_piece_shape, ghost_piece_orientation)) {
			ghost_piece_y++;
		}
	}
}
...
static bool is_on_ground() {
	if (!verify_piece_placement(active_piece_x, active_piece_y + 1, active_piece_shape, active_piece_orientation)) {
		return true;
	} else {
		return false;
	}
...
void game_paint(unsigned __unused dt_usec)
{
	...
	for (int y = 0; y <= 3; y++) {
		for (int x = 0; x <= 3; x++) {
			if (lookup_rotation_table(x, y, ghost_piece_shape, ghost_piece_orientation) == 1)
				draw_mino(ghost_piece_x + x, ghost_piece_y + y, piece_colors[active_piece_shape], true);
		}
	}
	...
	for (int y = 0; y <= 3; y++) {
		for (int x = 0; x <= 3; x++) {
			if (lookup_rotation_table(x, y, active_piece_shape, active_piece_orientation) == 1)
				draw_mino(active_piece_x + x, active_piece_y + y, piece_colors[active_piece_shape], false);
		}
	}
	...
	for (int y = 0; y <= 3; y++) {
		for (int x = 0; x <= 3; x++) {
			if (lookup_rotation_table(x, y, active_bag[next_piece], 0) == 1)
				draw_mino(NEXT_LIST_POS_X + x, NEXT_LIST_POS_Y + y, piece_colors[active_bag[next_piece]], false);
		}
	}
	...
	for (int i = 1; i <= 3; i++) {
		...
		if (!use_future_bag) {
			for (int y = 0; y <= 3; y++) {
				for (int x = 0; x <= 3; x++) {
					if (lookup_rotation_table(x, y, active_bag[next_piece], 0) == 1)
						draw_mino(NEXT_LIST_POS_X + x, NEXT_LIST_POS_Y + y + (i * 4), piece_colors[active_bag[next_piece]], false);
				}
			}
		} else {
			for (int y = 0; y <= 3; y++) {
				for (int x = 0; x <= 3; x++) {
					if (lookup_rotation_table(x, y, future_bag[next_piece], 0) == 1)
						draw_mino(NEXT_LIST_POS_X + x, NEXT_LIST_POS_Y + y + (i * 4), piece_colors[future_bag[next_piece]], false);
				}
			}
		}
	}
		...
		for (int y = 0; y <= 3; y++) {
			for (int x = 0; x <= 3; x++) {
				if (lookup_rotation_table(x, y, hold_piece, 0) == 1)
					draw_mino(HOLD_PIECE_POS_X + x, HOLD_PIECE_POS_Y + y, piece_colors[hold_piece], false);
			}
		}
	}
}
