...
static int level_speeds[11] = { 1000000, 500000, 250000, 125000, 62500, 31250,
				15625,	 7312,	 3656,	 1578,	 789 };
static int max_level = 10; // always the number of levels - 1
...
static void draw_mino(int x, int y, int color, bool ghost)
{
	if (color != 0) {
		if (!ghost) {
			sdk_draw_tile(SPACE_SIZE * x + BOARD_OFFSET_X,
				      SPACE_SIZE * y + BOARD_OFFSET_Y, &ts_pieces_png, color - 1);
		} else {
			sdk_draw_tile(SPACE_SIZE * x + BOARD_OFFSET_X,
				      SPACE_SIZE * y + BOARD_OFFSET_Y, &ts_pieces_png, 10);
		}
	}
}
...
void game_input(unsigned dt_usec)
{
	...
	if (time_after_falling > gravity_speed) {
		while (time_after_falling > gravity_speed) {
